...
// Read template
const templatePath = path.join(__dirname, '../src/pages/hebergement-web-ville.astro');
...
moroccanCitiesMaps.forEach((city) => {
  ...
  content = content.replace(/Hébergement web Ville/g, replaceVilleWithCity("Hébergement web Ville", cityName));
  content = content.replace(/Hébergement Web Ville/g, replaceVilleWithCity("Hébergement Web Ville", cityName));
  content = content.replace(/Hébergement Web ville/g, replaceVilleWithCity("Hébergement Web ville", cityName));
  content = content.replace(/à Ville/g, replaceVilleWithCity("à Ville", cityName));
  content = content.replace(/de Ville/g, replaceVilleWithCity("de Ville", cityName));
  content = content.replace(/à Ville/g, replaceVilleWithCity("à Ville", cityName));
  content = content.replace(/Ville\?/g, replaceVilleWithCity("Ville?", cityName));
  content = content.replace(/Ville !/g, replaceVilleWithCity("Ville !", cityName));
  content = content.replace(/Ville\?/g, replaceVilleWithCity("Ville?", cityName));
  content = content.replace(/Ville,/g, replaceVilleWithCity("Ville,", cityName));
  content = content.replace(/Ville\./g, replaceVilleWithCity("Ville.", cityName));
  content = content.replace(/Ville :/g, replaceVilleWithCity("Ville :", cityName));
  content = content.replace(/Ville et/g, replaceVilleWithCity("Ville et", cityName));
  ...
  // Fix the Taounate reference in FAQ (it's hardcoded)
  if (cityName !== "Taounate") {
    content = content.replace(/entreprises de Taounate/g, `entreprises de ${cityName}`);
  }
