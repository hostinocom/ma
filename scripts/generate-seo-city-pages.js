...
// Read template
const templatePath = path.join(__dirname, '../src/pages/agence-seo-ville.astro');
...
moroccanCitiesMaps.forEach((city) => {
  ...
  const fileName = `agence-seo-${cityUrlName}.astro`;
  ...
  content = content.replace(
    /const canonical_url = "https:\/\/www\.hostino\.ma\/agence-seo-ville";/g,
    `const canonical_url = "https://www.hostino.ma/agence-seo-${cityUrlName}";`
  );
  
  // Replace NationalCoverage props - handle both {""} and "" formats
  content = content.replace(
    /titleMap=\{""\}/g,
    `titleMap="${city.title}"`
  );
...
console.log('\nAll SEO agency city pages generated successfully!');
